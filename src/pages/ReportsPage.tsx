...
const ReportsPage: React.FC = () => {
  ...
  const [sortOrder, setSortOrderState] = useState<"desc" | "asc">(() => {
    return (
      (localStorage.getItem("reportSortOrder") as "desc" | "asc") || "desc"
    );
  });
  ...
  const formatPreview = (data: any) => {
    ...
    const previewLines = [
      ...
      previewSettings.showProject && selectedProjects.length > 0
        ? `Project: ${(selectedProjects as string[])
            .map((p: string) => p.trim())
            .join(" & ")}`
        : "",
  ...
  const handleExport = (key: string) => {
    ...
    const exportSettings = JSON.parse(
    ...
    autoTable(doc, {
      ...
      columnStyles: {
        ...
        ...(includeTime
          ? {
              [includeID ? (includeStatus ? 4 : 3) : includeStatus ? 3 : 2]: {
                cellWidth: 30,
              },
            }
          : {}),
  ...
  const calculateSummary = (reports: any[]) => {
    ...
    const extraHours =
      totalWorkedHours > actualWorkingHours
        ? totalWorkedHours - actualWorkingHours
        : 0;
    const lessHours =
      totalWorkedHours < actualWorkingHours
        ? actualWorkingHours - totalWorkedHours
        : 0;
    ...
    return {
      totalHours,
      totalMinutes,
      actualWorkingHours: formatTime(
        Math.floor(actualWorkingHours),
        (actualWorkingHours % 1) * 60
      ),
  ...
  useEffect(() => {
    ...
    handleDateRangeChange(null, [defaultStart, defaultEnd]);
  }, []);
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.ctrlKey && e.shiftKey && e.key.toLowerCase() === "c") {
        ...
        if (reportData.length > 0) {
          handleCopy(reportData[0].data, 0);
        }
      }
    };
  ...
  const filteredReports = reportData.filter((report) => {
    ...
    const taskMatch = tasks.some((task: any) => {
      const taskText = `${task.title || ""} ${task.status || ""} ${task.taskId || ""} ${task.status || ""} ${
        task.hours || ""
      } ${task.minutes || ""}`.toLowerCase();
      const subtaskMatch = task.subtasks?.some((sub: any) =>
        `${sub.title || ""} ${sub.status || ""}  ${task.taskId || ""} ${task.status || ""} ${sub.hours || ""} ${
          sub.minutes || ""
        }`
          .toLowerCase()
          .includes(query)
  ...
  return (
    <div className="reports-page">
      <div className="reports-header sticky-header">
        <h2>Reports</h2>
        <div
          className="reports-header-controls"
            ...
            return (
              <div style={{ display: "flex", gap: "20px", flexWrap: "wrap" }}>
                ...
                <p>
                  ...
                  <span style={{ color: workedHoursColor }}>
                    {" "}
      ...
      <div className="reports-content">
        {(filteredReports.length === 0 || !selectedDateRange) && (
          ...
          >
            {!selectedDateRange && (
              <p
                style={{ color: "#888", fontSize: "18px", fontWeight: "bold" }}
              >
          ...
          {filteredReports.map((report: any, index: number) => (
            <div key={index} className="report-card">
              <div className="task-preview-header">
                <h3>{`Date: ${dayjs(report.date, "YYYY-MM-DD").format(
                  "DD-MM-YYYY"
                )}`}</h3>
                <div
